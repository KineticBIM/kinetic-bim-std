...
operations:
    ...
    load_for_doc(doc)
        ...
        (scan_options, profiles). Search order:
            1. <dirname(doc.PathName)>/.bim/auto_tag.json
            2. <extension>/lib/annotation_qa/configs/default.json
    ...
    save_for_doc(doc, scan_options, profiles)
        ...
        auto_tag.json. Returns the path written, or None when the doc
        is unsaved (no PathName, so nowhere to put a project config).
        Errors are logged and swallowed: a failed save should never
        block the user from closing the dialog.
...
from annotation_qa import rules


PROJECT_CONFIG_FILENAME = "auto_tag.json"
...
def extension_default_path():
    """<extension>/lib/annotation_qa/configs/default.json - always
...
def project_config_path(doc):
    """<dirname(doc.PathName)>/.bim/auto_tag.json, or None when the doc
    has no path yet (unsaved / detached models)."""
    if doc is None:
        return None
    try:
        pn = doc.PathName
...
def save_for_doc(doc, scan_options, profiles, logger=None):
    """Persist scan_options + profiles to the project's auto_tag.json.
